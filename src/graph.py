...
from src.assistant import Assistant, main_assistant_runnable, delete_tasks_assisant_runnable
from src.assistant_tools import main_tools, delete_tasks, get_all_tasks, ToDeleteTasks
...
def route_delete_tasks(state: State):
    ...
    tool_calls = state["messages"][-1].tool_calls
    if tool_calls:
        if any(tc["name"] == get_all_tasks.name for tc in tool_calls):
...
def create_graph():
    ...
    builder.add_node("to_delete_tasks_assistant", ToolNode([ToDeleteTasks]))
    ...
    builder.add_edge("to_delete_tasks_assistant", "main_assistant")
    ...
    builder.add_node("delete_tasks", ToolNode([delete_tasks]))
    builder.add_node("get_all_tasks", ToolNode([get_all_tasks]))
    ...
    builder.add_edge("delete_tasks", "delete_tasks_assistant")
    builder.add_edge("get_all_tasks", "delete_tasks_assistant")
